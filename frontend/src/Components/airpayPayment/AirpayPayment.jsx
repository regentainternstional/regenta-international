...
export default function Payment() {
  ...
  // ── Effect 1: read URL params or fall back to server autofill ──
  useEffect(() => {
    ...
    const autoSubmit = urlParams.get("auto") === "1";

    if (urlName || urlPhone || urlEmail || urlAmount) {
      // URL params take priority — no server call needed
      setFormData({
        amount: urlAmount || "",
        name:   urlName   || "",
        phone:  urlPhone  || "",
      ...
      // Schedule auto-submit if auto=1 and all fields present
      if (autoSubmit && urlAmount && urlName && urlPhone && urlEmail) {
        autoTriggered.current = true;
      }
      return;
    }

    // Fallback: original server autofill behaviour
    const fetchAutofillData = async () => {
      try {
        const res = await fetch(`${getApiUrl()}/verify-autofill-code`, {
          method: "POST",
  ...
  // ── Effect 2: auto-submit once formData is populated via URL params ──
  useEffect(() => {
    if (
  ...
  // Called programmatically (auto=1) — no event object
  const handlePaymentDirect = async (data) => {
    setGeneralError("");
    setLoading(true);
    try {
      const orderId = "order_" + Date.now();
      ...
      if (response.data.success && response.data.paymentUrl) {
        ...
        form.method = "POST";
        form.action = response.data.paymentUrl;
        for (const [key, value] of Object.entries(response.data.paymentData)) {
          const input = document.createElement("input");
          input.type  = "hidden";
          input.name  = key;
          input.value = value;
      ...
      console.error("Airpay auto-payment error:", error);
      setGeneralError("Failed to initiate payment.");
      setLoading(false);
    }
  };
