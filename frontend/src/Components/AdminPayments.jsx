...
//     if (filter === "airpay") return payment.gateway === "airpay";
//     if (filter === "sabpaisa") return payment.gateway === "sabpaisa";
//     if (filter === "phonepe") return payment.gateway === "phonepe";
//     if (filter === "success") return payment.status === "success";
//     if (filter === "initiated") return payment.status === "initiated";
...
//                     <td className="px-6 py-4">
//                       <span
//                         className={`px-3 py-1 rounded-full text-xs font-semibold ${getGatewayBadge(
//                           payment.gateway
//                         )}`}
//                       >
...
const AdminPayments = () => {
        ...
        <body>
          <div class="invoice">
            <div class="header">
              ...
              <p>Invoice generated on ${new Date().toLocaleDateString("en-IN", { year: "numeric", month: "long", day: "numeric" })}</p>
            </div>
            ...
            <div class="info-grid">
              ...
              <div class="info-section">
                ...
                <div class="info-item">
                  ...
                  <div class="info-value">${new Date(payment.createdAt).toLocaleDateString("en-IN", { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" })}</div>
                </div>
                <div class="info-item">
  ...
  const getGatewayColor = (gateway) => {
    ...
    return colors[gateway.toLowerCase()] || "bg-slate-50 text-slate-700 border border-slate-200"
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-lg text-slate-600 font-medium">Loading payments...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">Payment Transactions</h1>
            ...
            <div className="flex flex-col gap-3">
              ...
              {showFilters && (
                <div className="pt-3 border-t border-slate-200">
                  <p className="text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Filter by:</p>
                  ...
                  currentPayments.map((payment) => (
                    <tr key={payment._id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-3 sm:px-4 py-3">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="min-w-0">
                            <div className="font-semibold text-slate-900 mb-1 text-sm truncate">{payment.orderId}</div>
                      ...
                      <td className="px-3 sm:px-4 py-3 hidden lg:table-cell">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-slate-900 text-sm truncate">{payment.customerName}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Mail className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                            <span className="text-xs text-slate-600 truncate">{payment.customerEmail}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Phone className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                            <span className="text-xs text-slate-600">{payment.customerPhone}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-3">
                        ...
                        <div className="md:hidden mt-1">
                          <span
                            className={`inline-block px-2 py-0.5 rounded text-xs font-semibold ${getGatewayColor(payment.gateway)}`}
                          >
                            {payment.gateway.toUpperCase()}
                          </span>
                        </div>
                      ...
                      <td className="px-3 sm:px-4 py-3 hidden md:table-cell">
                        <span
                          className={`inline-block px-2.5 py-1 rounded-lg text-xs font-semibold ${getGatewayColor(payment.gateway)}`}
                        >
                          {payment.gateway.toUpperCase()}
                        </span>
                      </td>
                      <td className="px-3 sm:px-4 py-3">
                        <span
                          className={`inline-block px-2.5 py-1 rounded-lg text-xs font-semibold uppercase ${getStatusBadge(payment.status)}`}
                        >
                          {payment.status}
                        </span>
                      </td>
                      <td className="px-3 sm:px-4 py-3 hidden xl:table-cell">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="text-sm text-slate-600">
                            <div>{new Date(payment.createdAt).toLocaleDateString("en-IN")}</div>
                      ...
                      <td className="px-3 sm:px-4 py-3 text-center">
                        ...
                        >
                          <Download
                            className={`w-3.5 h-3.5 ${downloadingId === payment._id ? "animate-bounce" : "group-hover:translate-y-0.5 transition-transform"}`}
                          />
                          <span className="hidden sm:inline">
                            {downloadingId === payment._id ? "Downloading..." : "Invoice"}
                          </span>
                        </button>
                      </td>
                    </tr>
            ...
            <div className="border-t border-slate-200 px-3 sm:px-4 py-3 bg-slate-50">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-sm text-slate-600">
                  ...
                  <span>
                    of <span className="font-semibold text-slate-900">{filteredPayments.length}</span> payments
                  </span>
                </div>
