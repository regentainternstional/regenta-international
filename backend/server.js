...
app.post("/api/sabpaisa/callback", async (req, res) => {
  try {
    ...
    if (!encData) {
      ...
      return res.redirect(
        `${process.env.FRONTEND_URL}/payment/failed?error=missing_data`,
      );
    }
    ...
    try {
      ...
      return res.redirect(
        `${process.env.FRONTEND_URL}/payment/failed?error=decryption_failed`,
      );
    }
    ...
    if (status && status.toUpperCase() === "SUCCESS") {
      ...
      res.redirect(
        `${process.env.FRONTEND_URL}/payment/failed?txnId=${transactionId}&status=${status}`,
      );
    }
  } catch (error) {
    ...
    res.redirect(
      `${process.env.FRONTEND_URL}/payment/failed?error=processing_error`,
    );
  }
});
...
app.get("/api/sabpaisa/callback", async (req, res) => {
  try {
    ...
    if (!encData) {
      console.error(
        "[v0] ❌ CRITICAL ERROR: No encData or encResponse in GET request",
      );
      return res.redirect(
        `${process.env.FRONTEND_URL}/payment/failed?error=missing_data`,
      );
...
app.post("/api/airpay/callback", async (req, res) => {
  try {
    ...
    if (!TRANSACTIONID) {
      ...
      console.error(
        "[v0] Expected URL: https://api.regentainternational.in/api/airpay/callback",
      );
      return res.redirect(
        `${process.env.FRONTEND_URL}/payment/failed?error=missing_txn_id`,
      );
    }
    ...
    if (txnhash.toString() !== ap_SecureHash) {
      ...
      return res.redirect(
        `${process.env.FRONTEND_URL}/payment/failed?error=invalid_hash`,
      );
    }
